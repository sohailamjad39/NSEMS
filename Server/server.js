...
app.use(
  cors({
    origin: [
      ...
      "https://localhost:3000", // Production build
...
app.get("/", (req, res) => {
  res.json({
    ...
    version: "1.0.0",
    description:
      "Secure, offline-capable digital student ID verification system",
    endpoints: {
      auth: "/api/auth/login",
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `║   📡 Network: http://${getIPAddress()}:${PORT}`.padEnd(58, " ") + "║",
  );
  console.log(
    `║   🛡️  Environment: ${process.env.NODE_ENV || "development"}`.padEnd(
      58,
      " ",
    ) + "║",
  );
